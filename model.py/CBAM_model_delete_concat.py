...
def model_simAM():
    ...
    x = _make_layer(Residual_Bottleneck, channel=16, name='block1')(x)       
    x = _make_layer(Residual_Bottleneck, channel=64, strides=2, two_times=True, name='block2')(x)   
    x = _make_layer(Residual_Bottleneck, channel=128, strides=2, two_times=True, name='block3')(x) 
    lstm = x
    lstm = layers.AveragePooling1D(pool_size=2, strides=2, padding='valid')(lstm) 
    lstm = layers.LSTM(64, dropout=0.3)(lstm) 
    x3 = tf.expand_dims(lstm, axis=1)       
    lstm = x3
