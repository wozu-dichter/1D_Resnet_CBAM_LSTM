...
def _make_layer(block, channel, strides=1, two_times=False, name=None):
    ...
    # layers_list.append(block(256, strides, two_times=two_times))
...
def model_CBAM():
    ...
    lstm = layers.Concatenate(axis=1)([x1, x2, x3]) # output: [N, 3, 64]
    lstm_add = layers.Add()([x1, x2, x3])           # output: [N, 1, 64]

    x = layers.AveragePooling1D(pool_size=2, strides=2, padding="same")(x)  # output: [N, 3, 256]
    global_pool = layers.GlobalAveragePooling1D()(x)    # output: [N, 256]
    global_pool = tf.expand_dims(global_pool, axis=1)   # output: [N, 1, 256]
    global_pool = global_pool*x                         # output: [N, 3, 256]

    concat_gloabal_lstm = layers.Concatenate(axis=-1)([lstm, global_pool]) # output:[N, 3, 320]
    
    con = layers.AveragePooling1D(pool_size=2, strides=2, padding='valid')(concat_gloabal_lstm) # output: [N, 1, 320]
    x = layers.Concatenate(axis=-1)([con, lstm_add])    # output: [N, 1, 384]

    x = ChannelAttention(384)(x)
    x = SpatialAttention()(x)

    ## fc layer
    x = layers.Dense(512, activation='relu')(x) # output:[N, 1, 320]
    x = layers.Dropout(0.5)(x)

    x = layers.Dense(256, activation='relu')(x) # output:[N, 1, 256]
    
    x = layers.Flatten()(x)                     # output:[N, 256]
    x = layers.Dense(81, activation='softmax', name='psd')(x) # output:[N, 81]
